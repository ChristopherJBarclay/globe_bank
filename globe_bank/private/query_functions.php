<?php
  ...
  function find_subject_by_id($id, $options=[]) {
    ...
    $sql = "SELECT * FROM subjects ";
    $sql .= "WHERE id='" . db_escape($db, $id) . "' "; // always add '' to id to prevent security issue
    if ($visible) {
  ...
  function shift_subject_positions($start_pos, $end_pos, $current_id=0) {
    ...
    // exlcude the current_id in the SQL WHERE clause
  ...
  function validate_page($page) {
    ...
    // menu_name
    // check for: isBlank && hasMoreThan2-lessThan255
    ...
    // position
    // is a number that exists already?
  ...
  function validate_admin($admin, $options=[]) {
    ...
    if(is_blank($admin['username'])) {
      ...
      $errors[] = "Username not allowed. Try again.";
    }
    ...
    if ($password_required) {
      if(is_blank($admin['password'])) {
        ...
        $errors[] = "Password must contain 12 or mroe characters.";
  ...
  function update_admin($admin) {
    ...
    // For UDPATE statements, $result is true/false
    if ($result) {
      return true;
    } else {
      // UPDATE failed
